...
describe('Helper functions', () => {
  ...
  describe('mergeUint8Arrays', () => {
    it('Works with two empty arrays', () => {
      expect(mergeUint8Arrays(new Uint8Array(), new Uint8Array())).toStrictEqual(new Uint8Array());
    });
    it('Works with two arrays', () => {
      expect(mergeUint8Arrays(Uint8Array.from([0, 0, 0]), Uint8Array.from([1, 1]))).toStrictEqual(
        Uint8Array.from([
          0, 0, 0, 1, 1
        ])
      );
      expect(mergeUint8Arrays(Uint8Array.from(Array.from({ length: 5 }, (_, i) => i)), Uint8Array.from(Array.from({length: 7}, (_, i) => (i+5)*3))))
        .toStrictEqual(
          Uint8Array.from([
            0, 1, 2, 3, 4,
    ...
    it('Works with multiple arrays', () => {
      expect(mergeUint8Arrays(Uint8Array.from([1]), Uint8Array.from([2]), Uint8Array.from([3])))
